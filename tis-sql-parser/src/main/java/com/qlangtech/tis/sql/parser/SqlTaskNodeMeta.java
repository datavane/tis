...
//        }
    ...
    public static class SqlDataFlowTopology implements IDataFlowTopology {
        ...
        public List<ColumnMetaData> getFinalTaskNodeCols() throws Exception {
            ...
            return colNames.stream().map((c) -> {
                return new ColumnMetaData(index.getAndIncrement(), c.getAliasName(), new DataType(JDBCTypes.VARCHAR),
                        false);
            }).collect(Collectors.toList());
        }
