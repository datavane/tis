...
import com.qlangtech.tis.realtime.transfer.{BasicPojoConsumer, DTO, IPk, IRowValueGetter, ITable}
import com.qlangtech.tis.realtime.transfer.impl.{CompositePK, DefaultPk, DefaultPojo, DefaultTable}
import com.qlangtech.tis.realtime.transfer.search4totalpay.S4totalpayListener._
  ...
  protected override def processColsMeta(builder: BuilderList): Unit = {
    ...
    cardBuilder.add( //
      ("id", "is_enterprise_card").c((row) => {
        ...
        putMediaResult("is_enterprise_card", //
          caseIfFunc(0,
            new Case(regexp(row.getColumn("id"), "^E_"), 1))
        )
        //===================================
        //===================================
    ...
    orderdetailBuilder.setGetterRowsFromOuterPersistence(/*gencode5*/
      (rowTabName, rvals, pk) => {
        ...
        orderdetailCriteria.addSelCol(OrderdetailColEnum.CODE, OrderdetailColEnum.ADDRESS, OrderdetailColEnum.SEAT_ID, OrderdetailColEnum.MOBILE, OrderdetailColEnum.END_TIME, OrderdetailColEnum.OPEN_TIME, OrderdetailColEnum.SEAT_CODE, OrderdetailColEnum.AREA_ID, OrderdetailColEnum.PEOPLE_COUNT, OrderdetailColEnum.ORDER_STATUS, OrderdetailColEnum.ORDER_FROM, OrderdetailColEnum.ORDER_KIND, OrderdetailColEnum.CUSTOMERREGISTER_ID, OrderdetailColEnum.IS_VALID, OrderdetailColEnum.INNER_CODE, OrderdetailColEnum.ORDER_ID, OrderdetailColEnum.TOTALPAY_ID, OrderdetailColEnum.ENTITY_ID)
        rowTabName match {
    ...
    payinfoBuilder.add( //
        ...
        breakable {
          for ((k: GroupKey, v: GroupValues) <- innertabAas) {
            putMediaResult("is_enterprise_card_pay", //
              caseIfFunc(0,
                new Case((sum(v, (r) => {
                  r.getColumn("is_enterprise_card_pay")
                }) > 0), 1))
            ) // end putMediaResult
            break
          }
        ...
        for ((k: GroupKey, v: GroupValues) <- payinfos) {
          v.putMediaData( //
            ...
            concat_ws("_", defaultVal(k.getKeyVal("kindpay_id"), "0"), typeCast("string", count(v, (r) => {
              1
            })), typeCast("string", round(sum(v, (r) => {
              r.getColumn("fee")
            }), 2)), typeCast("string", sum(v, (r) => {
              ((defaultDoubleVal(r.getColumn("coupon_fee"), 0) - defaultDoubleVal(r.getColumn("coupon_cost"), 0)) * defaultDoubleVal(r.getColumn("coupon_num"), 0))
            })), defaultVal(typeCast("string", min(v, (r) => {
              r.getColumn("pay_id")
            })), "0"))
          ) // end putMediaData
      ...
      , ("fee", "all_pay_fee").t((row, fieldValue) => {
        ...
        for ((k: GroupKey, v: GroupValues) <- payinfos) {
          ...
          import com.qlangtech.tis.extension.TISExtension
          import com.qlangtech.tis.ibatis.RowMap
          import com.qlangtech.tis.realtime.transfer.{BasicPojoConsumer, IPk, IRowValueGetter, ITable}
          import com.qlangtech.tis.realtime.transfer.impl.{CompositePK, DefaultPk, DefaultPojo, DefaultTable}
          import com.qlangtech.tis.realtime.transfer.search4totalpay.S4totalpayListener._
          import com.qlangtech.tis.sql.parser.tuple.creator.EntityName
          import com.google.common.collect.Lists
                ...
                ("draw_status", "has_fetch").t((row, fieldValue) => {
                  ...
                  for ((k: GroupKey, v: GroupValues) <- instancedetails) {
                    v.putMediaData( //
                      ...
                      caseIfFunc(0,
                        new Case((sum(v, (r) => {
                          op_and(r.getColumn("draw_status"), 8)
                        }) > 0), 1))
                    ) // end putMediaData
                  ...
                  breakable {
                    for ((k: GroupKey, v: GroupValues) <- instancedetails) {
                      result = concat_ws(",", collect_set(v, (r) => {
                        getArrayIndexProp(split(r.getColumn("batch_msg"), "[\\w\\W]*\\|"), 1)
                      }))
                      break
