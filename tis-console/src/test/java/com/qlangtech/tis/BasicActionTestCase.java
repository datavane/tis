...
public class BasicActionTestCase extends StrutsSpringTestCase implements TISEasyMock {
  ...
  protected void setCollection(String collection) {
    ...
    AppAndRuntime app = new AppAndRuntime(request.getParameterMap());
    DataXName dataX = DataXName.createDataXPipeline(collection);
    app.setAppName(dataX);
  ...
  protected static final String replica_core_url = "http://192.168.28"
    + ".200:8080/solr/search4employees_shard1_replica_n1/";
  public static final String name_shard1 = "shard1";

  //  protected DocCollection buildDocCollectionMock(boolean getSlicesMap, boolean fetchCollectionState, String
  //  collectionName, TISZkStateReader tisZkStateReader) throws Exception {
  ...
  //  protected IExpectationSetters<byte[]> createCoordinatorMock(Consumer<ITISCoordinator> consumer) throws
  //  IOException {
  //    return createCoordinatorMock(true, consumer);
  //  }
  //
  //  protected IExpectationSetters<byte[]> createCoordinatorMock(boolean overseer_elect_leader,
  //  Consumer<ITISCoordinator> consumer) throws IOException {
  ...
  //      try (InputStream input = this.getClass().getResourceAsStream("/com/qlangtech/tis/overseer_elect_leader
  //      .json")) {
