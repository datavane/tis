...
public class diff_match_patch_test extends TestCase {
    ...
    public void testBaisui() throws Exception {
        String str1 = IOUtils.toString(this.getClass().getResourceAsStream("/schema.xml"));
        String str2 = IOUtils.toString(this.getClass().getResourceAsStream("/schema2.xml"));
        LinkedList<Diff> differ = dmp.diff_main(str1, str2, false);
        StringBuffer result = new StringBuffer();
        for (Diff d : differ) {
            if (d.operation == Operation.EQUAL) {
                result.append(StringEscapeUtils.escapeXml(d.text));
            } else if (d.operation == Operation.DELETE) {
                result.append("<strike>").append(StringEscapeUtils.escapeXml(d.text)).append("</strike>");
            } else if (d.operation == Operation.INSERT) {
                result.append("<span class=\"add\">").append(StringEscapeUtils.escapeXml(d.text)).append("</span>");
            }
        }
    ...
    public void testDiff() {
        ...
        LinkedList<Diff> differ = dmp.diff_main(s1, s2, true);
        for (Diff d : differ) {
            if (d.operation == Operation.EQUAL) {
                compare.append(StringEscapeUtils.escapeXml(d.text));
            } else if (d.operation == Operation.DELETE) {
                compare.append("<span style='text-decoration:line-through;background-color:pink;'>").append(StringEscapeUtils.escapeXml(d.text)).append("</span>");
            } else if (d.operation == Operation.INSERT) {
                compare.append("<span style=\"background-color:#00FF00;\">").append(StringEscapeUtils.escapeXml(d.text)).append("</span>");
            }
        }
