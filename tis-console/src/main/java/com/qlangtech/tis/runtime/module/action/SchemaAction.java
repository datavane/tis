...
import com.qlangtech.tis.runtime.module.misc.TokenizerType.TokenizerString;
...
public class SchemaAction extends BasicModule {
    ...
    private boolean validateStupidContent(Context context, UploadSchemaWithRawContentForm form) {
        List<FieldErrorInfo> fieldsErrors = new ArrayList<FieldErrorInfo>();
        ...
        for (SchemaField f : form.getFields()) {
            if (StringUtils.isBlank(f.getName())) {
                ...
                FieldErrorInfo err = new FieldErrorInfo(f.getId());
                err.setFieldInputBlank(true);
                ...
                if (!duplicate.add(f.getName())) {
                    ...
                    if (!PATTERN_FIELD.matcher(f.getName()).matches() && !f.isDynamic()) {
                        ...
                        FieldErrorInfo err = new FieldErrorInfo(f.getId());
                        err.setNamePatternError(hasNamePatternError);
                        fieldsErrors.add(err);
                    } else if ("string".equalsIgnoreCase(f.getFieldtype()) && (StringUtils.isBlank(f.getTokenizerType()) || "-1".equalsIgnoreCase(f.getTokenizerType()))) {
        ...
        if (hasBlankError || hasFieldTypeError || hasDuplicateError || hasDuplicateError) {
            setBizResult(context, fieldsErrors);
            return false;
        }
        return true;
    }
    ...
    protected void serialTypes(final com.alibaba.fastjson.JSONObject schema, Collection<String> schemaTypeKeys) throws JSONException {
        ...
        for (Map.Entry<String, VisualType> t : TokenizerType.visualTypeMap.entrySet()) {
            ...
            if (t.getValue().isSplit()) {
                for (TokenizerType tokenType : ((TokenizerString) t.getValue()).getTokenerTypes()) {
    ...
    public static class VisualType {
        ...
        /**
         * @param type
         * @param ranageQueryAware 这个值只是说明可以选择
         * @param split
         */
        public VisualType(String type, boolean ranageQueryAware, boolean split) {
            super();
            this.type = type;
        ...
        public boolean isRanageQueryAware() {
            return ranageQueryAware;
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (this.ranageQueryAware && getRangedFieldName().equals(((VisualType) obj).type)) {
                return true;
            }
            return false;
        }

        public String getRangedFieldName() {
            if (this.ranageQueryAware) {
                return ('t' + type);
            }
            throw new IllegalStateException("ranageQueryAware:" + ranageQueryAware);
        }
    }

    public static class NumericVisualType extends VisualType {

        public static NumericVisualType create(VisualType type, boolean rangeEnable) {
            NumericVisualType result = new NumericVisualType(type.getType(), type.isRanageQueryAware(), type.isSplit());
        ...
        private NumericVisualType(String type, boolean ranageQueryAware, boolean split) {
            super(type, ranageQueryAware, split);
        }
