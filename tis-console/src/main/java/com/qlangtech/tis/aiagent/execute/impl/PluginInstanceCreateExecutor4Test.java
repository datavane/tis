 ...
 * @date 2025/12/3
 */
public class PluginInstanceCreateExecutor4Test extends BasicStepExecutor {
  @Override
  public boolean execute(TaskPlan plan, TaskStep step, AgentContext context) {
    Context ctx = plan.getRuntimeContext(false);
    IdentityName primaryFieldVal = IdentityName.create("tdfs_to_mysql_1");
    PartialSettedPluginContext pluginCtx = createPluginContext(plan, DataXName.createDataXPipeline(primaryFieldVal.identityValue()));
