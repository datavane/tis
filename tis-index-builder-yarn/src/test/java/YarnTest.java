...
//
//	public void run(CommandLine cli, Configuration hadoopConf)
//			throws Exception {
//		logCli(cli);
//		YarnConfiguration conf = (hadoopConf != null)
//				? new YarnConfiguration(hadoopConf) : new YarnConfiguration();
//		this.conf = conf;
//
...
//		amContainer
//				.setCommands(Collections.singletonList("$JAVA_HOME/bin/java"
//						+ " -Xmx128M" + " org.apache.solr.cloud.yarn.SolrMaster"
//						+ " -port=" + cli.getOptionValue("port", "8983")
//						+ " -zkHost=" + zkHost
//						+ " -nodes=" + Integer.parseInt(
//								cli.getOptionValue("nodes", "1"))
//						+ " -memory=" + cli.getOptionValue("memory", "512")
//						+ " -virtualCores="
//						+ cli.getOptionValue("virtualCores", "2") + " -solr="
//						+ cli.getOptionValue("solr") + confRes + hdfsHome
...
//
//		LocalResource solrAppJarLocalResource = Records
//				.newRecord(LocalResource.class);
//		setupSolrAppJar(new Path(cli.getOptionValue("jar")),
//				solrAppJarLocalResource);
...
//	public static void pingSolrCluster(String zkHost, int maxRetries)
//			throws Exception {
...
//	}
	...
	// */
	// public static Map<String, Object> getJson(String getUrl, int maxRetries)
	// throws Exception {
...
//	/**
//	 * Utility function for sending HTTP GET request to Solr with built-in retry
//	 * support.
//	 */
//	public static Map<String, Object> getJson(HttpClient httpClient,
//			String getUrl, int attempts) throws Exception {
//		Map<String, Object> json = null;
//		if (attempts >= 1) {
//			try {
//				HttpGet httpGet = new HttpGet(new URIBuilder(getUrl)
//						.setParameter("wt", "json").build());
...
//					log.warn("Request to " + getUrl + " failed due to: "
//							+ exc.getMessage()
...
//			if (entity != null) {
//				Object resp = ObjectBuilder
//						.getVal(new JSONParser(EntityUtils.toString(entity)));
//				if (resp != null && resp instanceof Map) {
//					return (Map<String, Object>) resp;
//				} else {
//					throw new ClientProtocolException(
//							"Expected JSON object in response but received "
//									+ resp);
//				}
//			} else {
...
//	}
//
//	protected void setupSolrAppJar(Path jarPath, LocalResource solrMasterJar)
//			throws IOException {
...
//	}
//
//	protected void setupAppMasterEnv(CommandLine cli,
//			Map<String, String> appMasterEnv) throws IOException {
...
//						new FileInputStream(extclasspathFile),
...
//							Environment.CLASSPATH.name(), part);
//			}
//		}
//	}
//
//	protected void logYarnDiagnostics(YarnClient yarnClient)
...
//					+ node.getNodeId() + ", nodeAddress" + node.getHttpAddress()
...
//				+ queueInfo.getApplications().size() + ", queueChildQueueCount="
...
//			optsDbg.append("\n");
//		}
//		log.info("Starting " + this.getClass().getSimpleName() + " with args: "
//				+ optsDbg);
//	}
//
...
//	}
//
//	public static void main(String[] args, Configuration conf)
//			throws Exception {
...
//		}
//
//		CommandLine cli = processCommandLineArgs(YarnTest.class.getName(),
//				getOptions(), args);
...
//						.withDescription(
//								"Application name; defaults to: SolrCloud")
//				.create("name"),
...
//						.withDescription(
//								"Address of the Zookeeper ensemble; defaults to: localhost:2181")
//						.create("zkHost"),
...
//						.withDescription(
//								"Solr HDFS home directory; if provided, Solr will store indexes in HDFS")
//						.create("hdfs_home"),
//				OptionBuilder.withArgName("JAR").hasArg().isRequired(true)
//						.withDescription(
//								"JAR file containing the SolrCloud YARN Application Master")
//						.create("jar"),
//				OptionBuilder.withArgName("ARCHIVE").hasArg().isRequired(true)
//						.withDescription(
//								"tgz file containing a Solr distribution.")
//						.create("solr"),
//				OptionBuilder.withArgName("INT").hasArg().isRequired(false)
//						.withDescription(
//								"Number of Solr nodes to deploy; default is 1")
//						.create("nodes"),
//				OptionBuilder.withArgName("INT").hasArg().isRequired(false)
//						.withDescription(
//								"Memory (mb) to allocate to each Solr node; default is 512M")
//						.create("memory"),
//				OptionBuilder.withArgName("INT").hasArg().isRequired(false)
//						.withDescription(
//								"Virtual cores to allocate to each Solr node; default is 1")
//						.create("virtualCores"),
//				OptionBuilder.withArgName("FILE").hasArg().isRequired(false)
//						.withDescription(
//								"Path to file containing additional classpath entries")
//						.create("extclasspath") };
//	}
//
//	/**
...
//	 */
//	public static CommandLine processCommandLineArgs(String app,
...
//			if (!hasHelpArg) {
//				System.err.println(
//						"Failed to parse command-line arguments due to: "
//								+ exp.getMessage());
//			}
//			HelpFormatter formatter = new HelpFormatter();
