...
public class NestAtomicUpdateProcessorFactory extends UpdateRequestProcessorFactory {
	...
	@Override
	public UpdateRequestProcessor getInstance(SolrQueryRequest request, SolrQueryResponse resp,
	...
	private class NestAtomicUpdateRequestProcessor extends UpdateRequestProcessor {
		...
		public void processAdd(AddUpdateCommand cmd) throws IOException {
			...
			if (YES.equals(solrDoc.getField(NEST_ATOMIC_UPDATE_TOKEN))) {
				...
				SolrInputDocument doc = RealTimeGetComponent.getInputDocumentFromTlog(req.getCore(), rootRef, null,
						null, true /* resolveFullDocument */);
				if (doc != null) {
					solrDoc = doc;
				} else {
					...
					try {
						...
						SolrInputDocumentRootQueryCollector rootCollector = new SolrInputDocumentRootQueryCollector(
								req.getCore(), rootRef);
			...
			for (IndexableField f : doc.getFields()) {
				...
				if (sf != null) {
					if ((!sf.hasDocValues() && !sf.stored()) || schema.isCopyFieldTarget(sf))
						continue;
					val = sf.getType().toObject(f); // object or external
													// string?
				} else {
					val = f.stringValue();
					if (val == null)
