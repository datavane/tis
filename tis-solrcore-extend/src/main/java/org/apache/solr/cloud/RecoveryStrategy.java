...
public class RecoveryStrategy implements Runnable, Closeable {
	...
	final private void recoveryFailed(final SolrCore core, final ZkController zkController, final String baseUrl,
	...
	final private void doReplicateOnlyRecovery(SolrCore core) throws InterruptedException {
		...
		// if (core.getUpdateHandler().getUpdateLog() != null) {
		// SolrException.log(log, "'replicate-only' recovery strategy should
		// only be used if no update logs are present, but this core has one: "
		// + core.getUpdateHandler().getUpdateLog());
		// return;
		// }
		while (!successfulRecovery && !Thread.currentThread().isInterrupted() && !isClosed()) { // don't
																								// use
																								// interruption
																								// or
																								// it
			...
			try {
				...
				boolean isLeader = leaderUrl.equals(ourUrl); // TODO: We can
																// probably
																// delete most
																// of this code
																// if we say
	...
	public final void doSyncOrReplicateRecovery(SolrCore core) throws Exception {
		...
		boolean firstTime = true;// replicaType != Replica.Type.TLOG;
		...
		while (!successfulRecovery && !Thread.currentThread().isInterrupted() && !isClosed()) { // don't
			...
			try {
				...
				// sendPrepRecoveryCmd(leader.getBaseUrl(),
				// leader.getCoreName(), slice);
		...
		// if replay was skipped (possibly to due pulling a full index from the
		// leader),
	...
	private final Replica pingLeader(String ourUrl, CoreDescriptor coreDesc, boolean mayPutReplicaAsDown)
		...
		while (true) {
			...
			if (!isClosed() && mayPutReplicaAsDown && numTried == 1 && docCollection
					.getReplica(coreDesc.getCloudDescriptor().getCoreNodeName()).getState() == Replica.State.ACTIVE) {
			...
			try (HttpSolrClient httpSolrClient = new HttpSolrClient.Builder(leaderReplica.getCoreUrl())
					.withSocketTimeout(1000).withConnectionTimeout(1000).build()) {
	...
	final private Future<RecoveryInfo> replay(SolrCore core) throws InterruptedException, ExecutionException {
