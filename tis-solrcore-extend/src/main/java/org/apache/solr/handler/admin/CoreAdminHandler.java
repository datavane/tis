...
public class CoreAdminHandler extends RequestHandlerBase implements PermissionNameProvider {
	...
	public void initializeMetrics(SolrMetricManager manager, String registryName, String tag, String scope) {
		...
		parallelExecutor = MetricUtils.instrumentedExecutorService(parallelExecutor, this,
				manager.registry(registryName),
	...
	protected boolean updateConfig(SolrQueryRequest req, SolrQueryResponse rsp, String collection, String cname,
		...
		try (SolrCore core = coreContainer.getCore(cname)) {
			...
			if (loalSnapshot == newSnapshotId) {
				String errorMsg = "repository snapshot is same to local snapshotid:" + loalSnapshot
						+ "shall not update config";
				log.warn(errorMsg);
	...
	public static ImmutableMap<String, String> paramToProp = ImmutableMap.<String, String>builder()
			...
			.put(CoreAdminParams.REPLICA_TYPE, CloudDescriptor.REPLICA_TYPE).build();
	...
	static class TaskObject {
		...
		public void setRspObject(SolrQueryResponse rspObject) {
			...
			this.backflowStatus = (IndexBackflowStatus) rspObject.getValues()
					.get(TisCoreAdminHandler.KEY_INDEX_BACK_FLOW_STATUS);
		}
...
//			postCoreAdminHandlerExecute(req, rsp, op);
	...
	interface CoreAdminOp {
		 ...
		 *            SolrException.ErrorCode.BAD_REQUEST ( 400 ) If the
		 *            execution of the command fails throw a SolrException with
		 *            SolrException.ErrorCode.SERVER_ERROR ( 500 )
		 * 
		 *            Any non-SolrException's are wrapped at a higher level as a
		 *            SolrException with SolrException.ErrorCode.SERVER_ERROR.
		 */
