...
//                }
        ...
        protected Class<?> findClass(String name) throws ClassNotFoundException {
            final List<PluginWrapper> deps = getTransitiveDependencies();
            if (PluginManager.FAST_LOOKUP) {
                for (PluginWrapper pw : deps) {
                    try {
...
//                }
            ...
            throw new ClassNotFoundException("by " + this._for.getName() + ",for:" + name + ",deps:"
                    + deps.stream().map((d) -> d.getShortName()).collect(Collectors.joining(",")));
        }
