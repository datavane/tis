...
public class AttrVals implements AttrValMap.IAttrVals {
    protected final Map<String, JSON> /*** attrName*/ attrValMap;
    ...
    public Map<String, JSONObject> asRootFormVals() {
        return this.attrValMap.entrySet().stream().collect(Collectors.toMap((e) -> e.getKey(), (e) -> (JSONObject) e.getValue()));
    }
    ...
    public Map<String, JSONArray> asSubFormDetails() {
        return this.attrValMap.entrySet().stream().collect(Collectors.toMap((e) -> e.getKey(), (e) -> (JSONArray) e.getValue()));
    }
