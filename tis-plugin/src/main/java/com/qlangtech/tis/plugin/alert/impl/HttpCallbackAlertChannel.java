    ...
    public void send(AlertTemplate alertTemplate) {
        ...
        try {
            ...
            if (this.sendAsJson != null && this.sendAsJson) {
                ...
                bodyMap.put("content", renderedContent);
                bodyMap.put("jobName", alertTemplate.getJobName());
                bodyMap.put("status", alertTemplate.getStatus());
                requestBody = JSON.toJSONString(bodyMap);
            }

            // 发送HTTP请求
            sendHttpRequest(this.callbackUrl, requestBody);
