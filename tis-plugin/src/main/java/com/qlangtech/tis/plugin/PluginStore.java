 ...
 * @date 2020/04/13
 */
public class PluginStore<T extends Describable> implements IRepositoryResource {
    ...
    public void copyConfigFromRemote() {
        // final URL url = CenterResource.getPathURL( //
        // Config.SUB_DIR_CFG_REPO + "/" + TIS.KEY_TIS_PLUGIN_CONFIG
        // + "/" + Descriptor.getPluginFileName(getSerializeFileName())
        // );
        // CenterResource.copyFromRemote2Local(url, file.getFile());
    ...
    public synchronized void copyFrom(PluginStore<T> other) {
        ...
        this.setPlugins(Collections.singletonList(parseDescribable));
    }

    public synchronized void setPlugins(List<Descriptor.ParseDescribable<T>> dlist) {
        try {
